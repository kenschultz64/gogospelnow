...
def translate(
    ...
    try:
        ...
        # Check 2: Length Ratio (Hallucination Guard)
        # If output is massively longer than input, it's likely a hallucination or instruction dump.
        # We only check if input is substantial enough (>2 words) to avoid false positives on short idioms.
        input_words = len(text.split())
        output_words = len(translation.split())
        
        if input_words > 2 and output_words > 20 and output_words > (input_words * 4):
             log_message(f"Discarded translation (Length Ratio Mismatch): Input ({input_words} words) vs Output ({output_words} words).", "WARNING")
             log_message(f" rejected text: {translation[:100]}...", "DEBUG")
             return None
        # -----------------------------------------
    ...
    if audio_data:
        ...
        if output_device_idx is not None:
            try:
                ...
                audio_player.play_audio(raw_data, output_device_idx, sample_rate, channels, sample_width)
...
class GlobalAudioPlayer:
    """Manages a persistent PyAudio session to avoid overhead and handle device changes."""
    def __init__(self):
        self.p = None
        self.stream = None
        self.last_rate = None
    ...
    def play_audio(self, raw_data, device_index, rate, channels, sample_width):
        """Plays audio data, automatically handling initialization and device errors."""
        import pyaudio
        
        # Retry loop: Try once, if it fails (device lost), reset and try again
        for attempt in range(2):
            try:
                with self._lock:
                    self._init_pyaudio()
                    
                    # Re-open stream if parameters changed or stream is missing
                    if (self.stream is None or 
                        self.last_rate != rate or 
                        self.last_channels != channels or 
                        self.last_device != device_index):
                        ...
                        format_fmt = self.p.get_format_from_width(sample_width)
                        log_message(f"Opening audio stream: Device={device_index}, Rate={rate}, Ch={channels}")
                    ...
                    log_message(f"Audio played through device {device_index}")
                    return # Success
                    
            except OSError as e:
                log_message(f"Audio device error (attempt {attempt+1}): {e}", "WARNING")
                # Force full reset of PyAudio
                self.close()
                if attempt == 1:
                    # If it failed twice, propagate error
                    raise e
            except Exception as e:
                log_message(f"Unexpected audio error: {e}", "ERROR")
                raise e

audio_player = GlobalAudioPlayer()
