...
uvicorn_server = None  # Will hold reference to uvicorn server for graceful shutdown
...
def translation_display_process_main(cmd_queue, initial_config, initial_text):
    ...
    def apply_config(cfg):
        ...
        if sys.platform.startswith("linux"):
            # On Linux, many window managers ignore geometry calls if the window 
            # is decorated and then transitions to undecorated. Setting override 
            # FIRST ensures it bypasses the WM placement logic immediately.
...
def tts_worker():
    ...
    while not tts_stop_event.is_set():
        try:
            ...
            # Safety: Truncate excessively long text to prevent worker blocking
            if len(translation) > 600:
                core.log_message(f"TTS text too long ({len(translation)} chars), truncating to preserve real-time sync.", "WARNING")
    ...
    with gr.Blocks(title="GoGospelNow.com") as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=200):
                ...
                # Shutdown button - below theme toggle, centered
                shutdown_btn = gr.Button(
                    "⏻ Shutdown App", 
                    variant="secondary",
                    elem_classes=["shutdown-btn"],
                    size="sm"
                )
                # Shutdown status display (will show fullscreen message when triggered)
                shutdown_status = gr.HTML(visible=True, elem_id="shutdown-status-display")
        gr.HTML(
            """
            <style>
            ...
            /* Shutdown button - orange to match app theme, centered */
            .shutdown-btn {
        ...
        def trigger_shutdown():
            ...
            import signal
            core.log_message("Shutdown requested via UI button...")
            
            # Set the shutdown event to signal server to stop
            shutdown_event.set()
            
            # Return a message before termination
            # The actual shutdown happens in a background thread to allow UI response
            def delayed_shutdown():
                time.sleep(1.0)  # Give time for browser close script to run
                ...
                try:
                    ...
                    core.log_message(f"Error during shutdown: {e}", "ERROR")
                
                core.log_message("Shutdown sequence complete. Exiting...")
            ...
            # Return simple HTML message (Gradio blocks JavaScript)
            return """
            <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); 
                        border-radius: 12px; color: white; margin-top: 10px;">
                <div style="font-size: 24px; font-weight: 700; margin-bottom: 8px;">
                    ✓ Application Shut Down
                </div>
                <div style="font-size: 16px; opacity: 0.95; margin-bottom: 12px;">
                    GoGospelNow has been safely stopped.
                </div>
                <div style="font-size: 14px; background: rgba(0,0,0,0.2); padding: 10px 15px; border-radius: 8px; display: inline-block;">
